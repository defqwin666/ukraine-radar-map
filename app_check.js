...
// regiony/rayony/hromady dataset: https://github.com/slawomirmatuszak/ukrainian_geodata citeturn8view0
const GEO_BASE = "https://raw.githubusercontent.com/slawomirmatuszak/ukrainian_geodata/main/";
const borderUrl = "https://raw.githubusercontent.com/EugeneBorshch/ukraine_geojson/master/UA_FULL_Ukraine.geojson"; // UA outline citeturn4view0
...
function makeIconForTarget(t, sizePx){
  ...
  const isShahed = type.includes("шахед") || type.includes("shahed");
...
function makeLabelHtml(t){
  ...
  const txt = count>1 ? `${name} ×${count}` : name;
...
function refreshTarget(t){
  ...
  const pausedTxt = t.paused ? " (пауза)" : "";
  ls.marker.bindPopup(`
    <div style="min-width:210px">
      <div style="font-weight:1000;margin-bottom:4px">${escapeHtml(title)}${pausedTxt}</div>
      <div style="font-size:12px;opacity:.85">Статус: ${statusText(t.status)}</div>
      <div style="font-size:12px;opacity:.85">Курс: ${t.heading ?? 0}° • Швидк: ${t.speed ?? 0} км/год</div>
      <div style="font-size:12px;opacity:.85">Загроза: R ${t.threatRadius ?? 0} км • ${t.threatAngle ?? 0}°</div>
      <div style="font-size:12px;opacity:.75">Тап по маркеру → меню</div>
    </div>
  `);
}
...
function openTargetMenu(t){
  ...
  const pausedLabel = t.paused ? "▶️ Зняти з паузи" : "⏸ Поставити на паузу";
  const choice = prompt(
`Дія для цілі:
1) ${pausedLabel}
2) ✏️ Редагувати
3) ➡️ Встановити новий напрямок (клік по мапі)
4) 📍 Додати точку повороту (клік по мапі)
5) 🌀 Телепорт (час/відстань)
6) 🧬 Клонувати
7) 🗑 Видалити
...
function editTarget(t){
  ...
  alert("Поля завантажені в панель. Зміни значення і натисни 'Застосувати до вибраної' у списку (кнопка знизу в карточці цілі).");
}
...
function startSetDirection(t){
  ...
  alert("Клікни на мапі, щоб задати новий напрямок.");
}
function startAddTurnPoint(t){
  pendingAction = { type:"addTurnPoint", targetId:t.id };
  alert("Клікни на мапі, щоб додати точку повороту.");
}
map.on("click", (e) => {
  if(state.addMode){
...
function teleportTarget(t){
  const mode = prompt("Телепорт: введи 't' (за часом) або 'd' (за відстанню):", "t");
  if(!mode) return;

  if(mode.toLowerCase()==="t"){
    const min = Number(prompt("Хвилини:", "1")||"0");
    const sec = Number(prompt("Секунди:", "0")||"0");
    ...
    const km = Number(prompt("Кілометри:", "0")||"0");
    const m  = Number(prompt("Метри:", "0")||"0");
  ...
  // mode after teleport
  const after = prompt("Режим після телепорту: 'move' або 'stop'", t.status==="stop"?"stop":"move");
...
function createTargetAt(latlng){
  ...
  $("btnAdd").textContent = "➕ Створити ціль";
}
...
// Speed tool (global multiplier like original “глобальний коеф. часу”)
...
function renderList(scrollToTop=true){
  ...
  if(!filtered.length){
    list.innerHTML = `<div class="hint">Поки що немає цілей.</div>`;
    return;
  }
    ...
    el.innerHTML = `
      <div class="itemTop">
        <div>
          ...
          <div class="hint">${escapeHtml(t.type)} • ${t.heading ?? 0}° • ${t.speed ?? 0} км/год • ID: ${(t._idText||"--")}${paused}</div>
        </div>
        <span class="badge"><span class="dot ${dot}"></span>${statusText(t.status)} • ${etaMin}</span>
      </div>

      <div class="actions">
        <button class="btn small" data-act="show" data-id="${t.id}">Показати</button>
        <button class="btn small" data-act="apply" data-id="${t.id}">Застосувати з панелі</button>
        <button class="btn small" data-act="dir" data-id="${t.id}">Напрямок</button>
        <button class="btn small danger" data-act="del" data-id="${t.id}">Видалити</button>
      </div>
    `;
    list.appendChild(el);
  }
  ...
  list.querySelectorAll("button[data-act]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      if(act==="apply"){
        // Apply target values to the panel (not наоборот)
        $("type").value = t.type || "Шахед";
        $("status").value = t.status || "move";
        $("name").value = t.name || "";
        $("count").value = (t.count ?? 1);
...
function estimateMinutes(t){
  // just a display like "0 хв."
  if(t.status==="stop" || (t.speed||0)<=0) return "0 хв.";
...
async function importJson(file){
  ...
  if(!obj?.data || !obj?.state) throw new Error("Невірний формат");
...
$("btnAdd").addEventListener("click", ()=>{
  ...
  $("btnAdd").textContent = state.addMode ? "🟢 Клікни на мапу…" : "➕ Створити ціль";
});
$("btnCenter").addEventListener("click", ()=> map.setView([48.3794, 31.1656], 6));
$("btnClear").addEventListener("click", ()=> { if(confirm("Видалити всі цілі?")) { data.targets=[]; save(); location.reload(); } });
$("btnClear2").addEventListener("click", ()=> { if(confirm("Видалити всі цілі?")) { data.targets=[]; save(); location.reload(); } });
...
$("file").addEventListener("change", async (e)=>{
  ...
  try{ await importJson(f); } catch(err){ alert("Помилка імпорту: " + err.message); }
  e.target.value="";
});
...
$("btnSpeed").addEventListener("click", ()=>{
  const v = Number(prompt("Глобальний коефіцієнт часу (0.5 - 2.0):", timeMultiplier.toFixed(2))||timeMultiplier);
  timeMultiplier = clamp(v, 0.5, 2.0);
  alert("Готово: ×" + timeMultiplier.toFixed(2));
});

$("btnWatermark").addEventListener("click", ()=>{
  const text = prompt("Текст водяного знака:", "ТЕКСТ");
  if(!text){ $("watermark").style.display="none"; return; }
  const density = Number(prompt("Щільність (1..5):", "2")||"2");
  const col = prompt("Колір (hex, напр #ffffff):", "#ffffff") || "#ffffff";
  const op = Number(prompt("Прозорість (0..1):", "0.20")||"0.2");
  const angle = Number(prompt("Кут (°):", "30")||"30");
  const pos = prompt("Позиція: center/tl/tr/bl/br", "center") || "center";
  drawWatermark(text, col, clamp(op,0,1), angle, clamp(density,1,5), pos);
  $("watermark").style.display="block";
});

$("btnStatic").addEventListener("click", ()=>{
  alert("Статичні об'єкти на оригіналі: ПУ Шахедів/аеродроми/морські бази/ПУ Балістики. Тут їх можна додати як GeoJSON через імпорт (у вкладці Інструменти: фронт/тривоги).");
});
...
$("alarmFile").addEventListener("change", async (e)=>{
  ...
  catch(err){ alert("Помилка GeoJSON: " + err.message); }
  e.target.value="";
});
...
function refreshAlarms(){
  ...
  if(state.alarmService==="ukrainealarm"){
    ...
    if(!(state.uaKey||"").trim()){
      alert("Потрібен UkraineAlarm API key.");
      return;
    }
    ...
    fetch("https://api.ukrainealarm.com/api/v3/alerts", {
      ...
      alert("Отримано дані, але для відмалювання потрібні полігони районів/областей, зв'язані з regionId. Це можна зробити, якщо ти даси таблицю відповідності ID↔полігон.");
    }).catch(()=>{
      alert("Не вийшло завантажити UkraineAlarm (може блокуватися CORS у браузері). Використай 'Свій GeoJSON (імпорт файлом)'.");
    });
  }
}
...
$("toggleMapTheme")?.addEventListener("click",()=>{
 if(!isDark){ if(typeof lightLayer!=="undefined") map.removeLayer(lightLayer); if(typeof darkLayer!=="undefined") darkLayer.addTo(map); $("toggleMapTheme").textContent="Світла мапа"; }
 else{ if(typeof darkLayer!=="undefined") map.removeLayer(darkLayer); if(typeof lightLayer!=="undefined") lightLayer.addTo(map); $("toggleMapTheme").textContent="Темна мапа<br><label><input type='checkbox' id='toggleShaft' checked> Палка шахеда</label>"; }
 isDark=!isDark;
});
...
(function(){
  ...
  inp.addEventListener("input", ()=>{
    ...
    tmr=setTimeout(async ()=>{
      try{
        const url = "https://nominatim.openstreetmap.org/search?format=json&limit=8&addressdetails=1&q="+encodeURIComponent(q+" Україна");
        ...
        show([{lat:"",lon:"",name:"Немає інтернету / пошук недоступний",desc:""}]);
      }
    }, 350);
  });
...
function pointIconHTML(p){
  ...
  if(kind==="img" && p.img){
    ...
    inner = `<div style="font-size:18px;">📍</div>`;
  }else{
...
function renderPoints(){
  ...
  for(const p of (state.points||[])){
    ...
    m.on("contextmenu", ()=>{
      if(confirm("Видалити точку '"+(p.name||"")+"'?")){
...
$("ptMode")?.addEventListener("click", ()=>{
  pointMode = !pointMode;
  $("ptMode").textContent = "📍 Режим додавання точки: " + (pointMode?"Увімк":"Вимк");
});
$("ptClear")?.addEventListener("click", ()=>{
  if(confirm("Очистити всі точки?")){
    state.points = [];
    save();
    renderPoints();
  }
...
map.on("click", (e)=>{
  if(!pointMode) return;
  const name = ($("ptName")?.value || "").trim() || "Точка";
...
function normOblastName(n){
  ...
  n = n.replace(/^м\.\s*/i,"").replace(/\s+обл\.?$/i," область");
  if(n.toLowerCase()==="крим" || n.toLowerCase().includes("крим")) return "Автономна Республіка Крим";
  if(!/область$/i.test(n) && !n.toLowerCase().includes("республіка")) n += " область";
  return n;
}
